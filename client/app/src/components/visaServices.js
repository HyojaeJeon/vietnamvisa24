...
import { Smartphone, Briefcase, CheckCircle, Star, Clock, Users, Building, Shield, ArrowRight, Award } from "lucide-react";
...
function VisaServices({ onApplyClick }) {
  ...
  const handleApplyClick = (serviceName) => {
    // 해당 서비스 데이터 찾기
    const serviceData = mainServices.find((service) => service.title === serviceName);
  ...
  const mainServices = [
    {
      ...
      features: ["24시간 빠른 처리", "단수/복수 선택 가능", "90일 체류 가능", "99.8% 승인률"],
      processing: "당일~5일",
      price: "89,000원부터",
      highlight: true,
    },
    {
      ...
      features: ["최대 1년 체류", "복수 입국 가능", "초청장 지원", "전문가 컨설팅"],
      processing: "7~14일",
      price: "150,000원부터",
      highlight: false,
    },
    {
      ...
      features: ["노동허가서 발급", "거주증 신청", "가족 동반 가능", "전 과정 지원"],
      processing: "30~60일",
      price: "상담 후 견적",
      highlight: false,
    },
  ...
  return (
    <section className="relative py-24 bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 overflow-hidden">
      ...
      <div className="absolute inset-0 overflow-hidden">
        <div className="absolute top-20 left-10 w-4 h-4 bg-blue-400/30 rounded-full animate-ping" style={{ animationDelay: "0s" }}></div>
        <div className="absolute top-40 right-20 w-3 h-3 bg-purple-400/30 rounded-full animate-ping" style={{ animationDelay: "2s" }}></div>
        <div className="absolute bottom-40 left-1/4 w-2 h-2 bg-emerald-400/30 rounded-full animate-ping" style={{ animationDelay: "4s" }}></div>
        <div className="absolute top-1/3 right-1/4 w-5 h-5 bg-orange-400/20 rounded-full animate-ping" style={{ animationDelay: "1s" }}></div>
      </div>
      <div className="container mx-auto px-6 relative z-10">
        ...
        <div className={`text-center mb-16 transform transition-all duration-1000 ${isVisible ? "translate-y-0 opacity-100" : "translate-y-10 opacity-0"}`}>
          ...
          <div className="inline-flex items-center space-x-2 bg-white/80 backdrop-blur-sm px-6 py-3 rounded-full border border-blue-200/50 mb-6 shadow-lg">
            ...
            <span className="text-blue-700 font-semibold">베트남 비자 전문 서비스</span>
          </div>
          {/* 메인 타이틀 */}
          <h2 className="text-4xl md:text-5xl lg:text-6xl font-bold mb-6 leading-tight">
            <span className="bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 bg-clip-text text-transparent">베트남 비자 서비스</span>
          </h2>
          {/* 서브 타이틀 */}
          <p className="text-xl md:text-2xl text-slate-600 max-w-4xl mx-auto leading-relaxed mb-8">
            목적에 맞는 최적의 베트남 비자를 선택하세요.
            <br />
            <span className="text-blue-600 font-semibold">AI 기술로 빠르고 정확한 처리를 보장합니다.</span>
          </p>
          {/* 통계 카드 */}{" "}
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-4xl mx-auto mb-12">
            {quickStats.map((stat) => (
              <div
                key={stat.label}
                className={`bg-white/80 backdrop-blur-sm border border-white/50 rounded-2xl p-4 text-center shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105 ${
        ...
        <div className="grid lg:grid-cols-3 gap-8 mb-16">
          {mainServices.map((service, index) => {
            ...
            return (
              <div
                key={service.id}
                className={`group relative transform transition-all duration-700 z-[99] flex ${isVisible ? "translate-y-0 opacity-100" : "translate-y-20 opacity-0"} ${
                  service.highlight ? "lg:scale-105" : ""
                }`}
              ...
              >
                <Card
                  className={`relative bg-white/90 backdrop-blur-sm border-0 shadow-2xl hover:shadow-2xl transition-all duration-500 w-full flex flex-col ${
                    service.highlight ? "ring-2 ring-yellow-400 ring-offset-4" : ""
                  } ${hoveredCard === service.id ? "scale-105 ring-2 ring-blue-300 ring-offset-2" : ""} z-[101]`}
                >
                  {/* 상단 그라디언트 바 */}
                  ...
                  <CardHeader className="pb-4 pt-8">
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center space-x-4">
                        ...
                        <div>
                          <CardTitle className="text-2xl text-slate-800 mb-1">{service.title}</CardTitle>
                          <p className="text-blue-600 font-semibold text-sm">{service.subtitle}</p>
                        </div>
                      </div>
                      <div className={`${service.badgeColor} text-white px-3 py-1 rounded-full text-xs font-semibold`}>{service.badge}</div>
                    </div>
                  ...
                  <CardContent className="space-y-6 flex-1 flex flex-col">
                    ...
                    <div className="bg-slate-50 rounded-xl p-4 space-y-3 mt-auto">
                      <div className="flex justify-between items-center">
                        ...
                        <span className="text-slate-800 font-semibold">{service.processing}</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-slate-600 font-medium">대행 수수료</span>
                        <div className="text-right">
                          <span className="text-2xl font-bold text-slate-800">{service.price}</span>
                          {service.price !== "상담 후 견적" && <div className="text-sm text-slate-500">VAT 포함</div>}
                        </div>
                      </div>
                    </div>
                    {/* 신청 버튼 - 하단 고정 */}
      ...
      {/* 챗봇 컴포넌트 */}
      {isChatbotOpen && <Chatbot initialMessage={chatbotMessage} isOpen={isChatbotOpen} onToggle={() => setIsChatbotOpen(false)} />}
    </section>
  );
}
