...
import { Car, Plane, Shield, CheckCircle, Star, MapPin, Sparkles } from "lucide-react";
...
function AdditionalServices({ onApplyClick }) {
  const services = [
    ...
    },
    {
      ...
      features: ["7일 미만", "7일 이상 14일 미만", "그 이후 상담"],
      price: "상담 문의",
    ...
    <section id="additional-services" className="py-24 bg-gradient-to-b from-white to-gray-50">
      <div className="container mx-auto px-4">
        <div className="text-center mb-20 animate-fade-in">
          ...
          <p className="text-gray-600 text-xl max-w-3xl mx-auto">비자 발급과 함께 필요한 다양한 서비스를 원스톱으로 제공합니다.</p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-6">
          {services.map((service, index) => {
            ...
            return (
              <div key={service.id} className={`group animate-fade-in ${service.popular ? "pt-6" : "pt-0"}`} style={{ animationDelay: `${index * 0.2}s` }}>
                <div className="relative h-full">
                  {service.popular && (
                    <div className="absolute -top-4 left-1/2 transform -translate-x-1/2 z-30">
                      <div className="bg-gradient-to-r from-green-500 to-emerald-500 px-2 md:px-4 py-1 md:py-2 rounded-full text-white text-xs md:text-sm font-bold shadow-lg">⭐ 인기</div>
                    </div>
                  )}
                  ...
                  <Card className="relative overflow-hidden floating-card gradient-card border-0 h-full flex flex-col z-10">
                    <CardHeader className="pb-2 md:pb-4 p-3 md:p-6 flex-shrink-0">
                      <div className="flex items-center justify-center mb-2 md:mb-4">
                        <div
                          className={`w-10 h-10 md:w-16 md:h-16 bg-gradient-to-br ${service.gradient} rounded-xl flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform duration-300`}
                        >
