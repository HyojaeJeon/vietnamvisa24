...
import { UPDATE_CONSULTATION_STATUS_MUTATION, DELETE_CONSULTATION_MUTATION } from "../../src/lib/graphql/mutation/consultations/index.js";
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-2">고객 문의를 효율적으로 관리하고 빠른 응답을 제공합니다.</p>
        </div>
        <div className="flex space-x-3">
          <Button className="bg-green-600 hover:bg-green-700">
            <Plus className="h-4 w-4 mr-2" />새 상담 등록
          </Button>
        </div>
      </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-yellow-600">{consultations.filter((c) => c.status === "pending").length}</p>
              </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-blue-600">{consultations.filter((c) => c.status === "in_progress").length}</p>
              </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-green-600">{consultations.filter((c) => c.status === "completed").length}</p>
              </div>
          ...
          <div className="flex flex-col md:flex-row space-y-4 md:space-y-0 md:space-x-4">
            <div className="flex-1">
              <div className="relative">
                ...
                <Input placeholder="고객명, 이메일, 서비스 유형으로 검색..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-10" />
              </div>
            </div>
            <div className="flex space-x-3">
              <select
      ...
      {filteredConsultations.length === 0 && (
        <Card>
          <CardContent className="p-12 text-center">
            ...
            <p className="text-gray-500">{searchTerm || statusFilter !== "all" ? "검색 조건에 맞는 상담이 없습니다." : "아직 접수된 상담이 없습니다."}</p>
          </CardContent>
        </Card>
      )}
                  ...
                  <div className="flex items-center space-x-6">
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center space-x-4">
                        <h3 className="text-lg font-semibold text-gray-900">{consultation.service_type || "일반 상담"}</h3>
                        <span className="text-sm text-gray-500">ID: {consultation.id}</span>
                      </div>
                      ...
                      <div className="flex items-center space-x-4">
                        <span className="px-2 py-1 bg-gray-100 text-gray-700 text-xs font-medium rounded-full">{getInquiryTypeLabel(consultation.service_type)}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center space-x-4">
                    ...
                    <div className="flex space-x-2">
                      ...
                      {consultation.status === "pending" && (
                        <Button size="sm" className="bg-blue-600 hover:bg-blue-700" onClick={() => handleStatusChange(consultation.id, "in_progress")}>
                          <Reply className="h-4 w-4 mr-1" />
                          처리 시작
                        </Button>
                      )}
                      {consultation.status === "in_progress" && (
                        <Button size="sm" className="bg-green-600 hover:bg-green-700" onClick={() => handleStatusChange(consultation.id, "completed")}>
            ...
            <div className="mt-6 pt-6 border-t">
              <div className="flex space-x-3">
                {selectedConsultation.status === "pending" && (
                  <Button className="bg-blue-600 hover:bg-blue-700" onClick={() => handleStatusChange(selectedConsultation.id, "in_progress")}>
                    처리 시작
                  </Button>
                )}
                {selectedConsultation.status === "in_progress" && (
                  <Button className="bg-green-600 hover:bg-green-700" onClick={() => handleStatusChange(selectedConsultation.id, "completed")}>
                    처리 완료
                  </Button>
                )}
                <Button variant="outline">답변 메일 보내기</Button>
