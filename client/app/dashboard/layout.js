...
export default function DashboardLayout({ children }) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50 flex">
      ...
      {isMobile && sidebarOpen && <div className="fixed inset-0 bg-black bg-opacity-50 z-40 transition-opacity duration-300" onClick={() => setSidebarOpen(false)} />}
      ...
      >
        ...
        <div className={`relative flex items-center justify-between h-20 px-4 bg-gradient-to-r from-blue-600 to-indigo-700 ${!sidebarOpen && !isMobile ? "px-2" : ""}`}>
          ...
          {!isMobile && (
            <button onClick={toggleSidebar} className="p-2 rounded-lg text-white hover:bg-white/20 transition-colors">
          ...
          {/* Mobile close button */}
          {isMobile && (
            <button onClick={() => setSidebarOpen(false)} className="p-2 rounded-lg text-white hover:bg-white/20 transition-colors">
              <X className="h-5 w-5" />
            </button>
          )}
        </div>
            ...
            <div className="flex items-center space-x-4">
              <div className="relative">
                <div className="w-14 h-14 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-xl flex items-center justify-center shadow-lg">
                  <span className="text-white font-bold text-lg">{data.adminMe.name.charAt(0)}</span>
                </div>
              ...
              <div className="flex-1">
                ...
                <span
                  className={`inline-flex items-center px-3 py-1 text-xs font-medium rounded-full ${
                    data.adminMe.role === "SUPER_ADMIN"
                      ? "bg-yellow-100 text-yellow-800 border border-yellow-200"
                      : data.adminMe.role === "MANAGER"
                      ? "bg-blue-100 text-blue-800 border border-blue-200"
                      : "bg-green-100 text-green-800 border border-green-200"
                  }`}
                >
                  {data.adminMe.role === "SUPER_ADMIN" ? "최고관리자" : data.adminMe.role === "MANAGER" ? "매니저" : "스태프"}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="px-2 py-4 border-b border-gray-200">
            <div className="flex justify-center">
              <div className="w-10 h-10 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-lg flex items-center justify-center shadow-lg">
                <span className="text-white font-bold text-sm">{data.adminMe.name.charAt(0)}</span>
              </div>
            </div>
          </div>
        )}
      ...
      <div className={`flex-1 flex flex-col min-h-screen transition-all duration-300 ${!isMobile && !sidebarOpen ? "ml-0" : ""}`}>
        ...
        <div className="sticky top-0 z-30 h-16 bg-white/90 backdrop-blur-md shadow-sm border-b border-gray-200 flex items-center justify-between px-6">
          <div className="flex items-center space-x-4">
            <button onClick={toggleSidebar} className="p-2 rounded-lg text-gray-500 hover:text-gray-700 hover:bg-gray-100 transition-all">
