...
export default function Dashboard() {
  ...
  const getActivityIcon = (type) => {
    switch (type) {
      ...
      case 'document': return <Eye className="h-4 w-4 text-green-500" />;
      case 'approval': return <CheckCircle className="h-4 w-4 text-green-600" />;
      case 'consultation': return <Users className="h-4 w-4 text-orange-500" />;
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900">
          안녕하세요, {adminData?.adminMe?.name || '관리자'}님! 👋
        </h1>
        <p className="text-gray-600 mt-2">
          오늘도 베트남 비자 서비스 운영을 위해 수고하고 계시네요.
        </p>
      </div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-right">
                    <span className={`text-sm font-medium ${
                      item.priority === 'urgent' ? 'text-red-600' : 'text-orange-600'
                    }`}>
                      {item.days_overdue ? `${item.days_overdue}일 지연` : 
                       item.days_left ? `${item.days_left}일 남음` :
                       `${item.hours_left}시간 남음`}
                    </span>
                  </div>
                </div>
              </CardContent>
