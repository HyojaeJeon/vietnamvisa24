...
const ServiceSelectionStep = ({ formData, onUpdate, onNext }) => {
  ...
  const handleAdditionalServiceToggle = (serviceKey, checked) => {
    const newAdditionalServices = checked 
      ? [...(formData.additionalServices || []), serviceKey]
      : (formData.additionalServices || []).filter(s => s !== serviceKey);
            ...
            {processingOptions.map((processing) => (
              ...
              >
                <div className="text-center space-y-2">
                  <div className="w-10 h-10 bg-gradient-to-r from-purple-400 to-pink-600 rounded-lg flex items-center justify-center text-white mx-auto">
                    {processing.icon}
                  </div>
                  ...
                  {processing.price > 0 && (
                    <p className="text-xs font-bold text-gray-800">+${processing.price}</p>
                  )}
