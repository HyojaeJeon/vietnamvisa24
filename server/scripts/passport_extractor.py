...
def extract_non_mrz_info(preprocessed_img):
    ...
    try:
        ...
        # 발급일: 'Date of issue' 또는 '발급일' 다음에 오는 날짜 형식
        # (예: 01 JAN 2023)
        issue_date_match = re.search(r'(?:Date of issue|발급일)\s*:?\s*(\d{2}\s[A-Z]{3}\s\d{4})', text, re.IGNORECASE)
        if issue_date_match:
...
def main(image_path):
    ...
    # 4. 두 결과 합치기
    final_output = {**mrz_data, **non_mrz_data}
